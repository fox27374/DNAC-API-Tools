...
def get_type(ip):
    # Get the device type and check if it is in the database
    exists = get_url("network-device/ip-address/%s" % ip)['response']['errorCode']
    if exists:
        return 'Device does not exist'
    else:
        return get_url("network-device/ip-address/%s" % ip)['response']['type']
...
if __name__ == "__main__":
    ...
    dev_type = get_type(args.ip)
    if 'exist' in dev_type:
        print(dev_type)
    elif 'witch' not in dev_type:
        print('Device is not a switch')
    else:
        dev_id = ip_to_id(args.ip)
        interfaces = get_interfaces(dev_id)
        hosts = get_hosts(args.ip)
